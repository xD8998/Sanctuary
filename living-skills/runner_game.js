...
import { showEmotionQuizUI } from './emotion_quiz.js';
...
export async function initGame(args={}) {
  ...
  if (!versionElGame) { versionElGame = document.createElement('div'); versionElGame.className='version-badge version-game'; versionElGame.textContent='V1.02 Original • LS: V1'; root.appendChild(versionElGame); }
}
...
function slide(dir){
  ...
  animateMove(path, async ()=>{
    ...
    if (!dev.antiRed && reds.some(r=> r.x===player.x && r.y===player.y)) {
      const di = reds.findIndex(r=> r.x===player.x && r.y===player.y);
      deathSfx(); await smallDelay(140);
      const timeLimitSec = (level > 5 && Math.random() < 0.3) ? [60,30,20,10][Math.floor(Math.random()*4)] : null;
      const el = showEmotionQuizUI({
        root,
        timeLimitSec,
        level,
        onCorrect: () => {
          if (di>=0) { removeAt(reds, reds[di].x, reds[di].y); }
          overlay?.remove(); overlay=null; moveLock=false; running=true; draw();
          tip('Question cleared! Red ghost destroyed.');
          raf = requestAnimationFrame(loop);
        },
        onWrong: (reason) => {
          overlay?.remove(); overlay=null; moveLock=false;
          if (reason === 'menu') { running=false; fadeOutAndExit(); }
          else if (reason === 'restart') { restartRun(); }
          else { running=false; showDeathMenuFromQuiz(reason); }
        }
      });
      overlay = el; running=false;
      return;
    }
    // orange ghost pickup
...
function maybeShowAdvancedTutorial(force=false){
  ...
  const basics = [
    ...
    ()=>{ const r=document.createElement('div'); r.innerHTML='<div><b>Red ghosts</b> — touching one asks a quick question. <b>Answer correctly</b> to destroy the red ghost and continue. <b>Wrong</b> or <b>run out of time</b> and your run resets to Level 1.</div>'; const cv=document.createElement('canvas'); cv.width=260; cv.height=260; cv.style.display='block'; cv.style.margin='12px auto'; renderGhostPreview(cv,'#ff7373'); r.appendChild(cv); return r; },
...
function showDeathMenuFromQuiz(reason='wrong'){
  const msg = reason === 'timeout'
    ? 'You ran out of time. Try answering the question next time.'
    : 'Wrong answer. Your run will reset to Level 1.';
  showOverlay(`
    <div style="text-align:center; display:grid; gap:10px;">
      <div style="font-weight:700;">${msg}</div>
      <div>Score ${score} — High ${highScore}${dev.enabled?' (DEV unsaved)':''}</div>
      <div style="display:flex; gap:8px; justify-content:center;">
        <button id="ovr-restart">Reset (Level 1)</button>
        <button id="ovr-menu">Back to Menu</button>
      </div>
    </div>
  `);
