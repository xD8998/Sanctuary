...
window.addEventListener('exit-game', async () => {
  ...
  await new Promise(r => fadeEl.addEventListener('transitionend', r, { once: true }));
  const mod = await import('./game.js'); try { mod.destroyGame(); } catch {}
  document.body.classList.remove('in-game');
  const gameRoot = document.getElementById('game-root'); gameRoot.innerHTML = ''; gameRoot.hidden = true;
  document.querySelector('.menu')?.classList.add('show');
  try { audioEl.currentTime = 46; audioEl.play().catch(()=>{}); if (started) { lowpass?.frequency.setTargetAtTime(18000, actx.currentTime, 0.2); fadeIn(1.2); } } catch {}
  fadeEl.classList.remove('show');
  const clear = () => fadeEl.setAttribute('aria-hidden','true');
  fadeEl.addEventListener('transitionend', clear, { once: true }); setTimeout(clear, 700);
});